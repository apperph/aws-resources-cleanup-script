...
def stop_rds_cluster(event, context):
    ...
    for account_id in account_ids:
        ...
        for region in regions:
            ...
            rds_region = boto3.client('rds', region_name=region,
                                      ...
                                      aws_secret_access_key=credentials['SecretAccessKey'],
                                      aws_session_token=credentials['SessionToken'])
            dbcluster = rds_region.describe_db_clusters()
            count=len(dbcluster['DBClusters'])
            print(f"{count} clusters are found running in {region} of account {account_id}")

            # Iterate through each databases to check instance launch time            
            for cluster in dbcluster['DBClusters']:
                for cluster_id in cluster['DBClusterIdentifier']:
                    launch_time = cluster['ClusterCreateTime']
...
def lambda_handler(event, context):
    stop_rds_cluster(event, context)
